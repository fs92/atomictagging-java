...
public class GenericImporter implements IMoleculeImporter {
	...
	public void importFile( final Collection<IMolecule> molecules, final File file, final String repository ) {
		...
		final AtomBuilder binRefBuilder = Atom.build().withData( "/" + fileName ).withTag( CoreTags.FILEREF_TAG )
				.withTag( CoreTags.FILETYPE_UNKNOWN );
		...
		final MoleculeBuilder mBuilder = Molecule.build().withAtom( filename )
				.withAtom( binRefBuilder.buildWithDataAndTag() ).withTag( "generic-file" );

		if ( isRemote ) {
			final IAtom remote = Atom.build().withData( repository ).withTag( CoreTags.FILEREF_REMOTE_LOCATION )
					.buildWithDataAndTag();
