...
public class Shell {
	...
	private static void showList(String input) {
		...
		for (IMolecule molecule : molecules) {
			...
			for (IAtom atom : molecule.getAtoms()) {
				...
				String data = (atom.getData().length() > 20) ? atom.getData().substring(0, 20) + "..." : atom.getData();
	...
	private static void writeTestData() {
		...
		DbWriter.write(Molecule.build().withAtoms(Arrays.asList(artist, title, album, rock, metal)).withTag("audio")
		...
		DbWriter.write(Molecule.build().withAtoms(Arrays.asList(artist, title, album, rock)).withTag("audio").withTag(
				"favorite").buildWithAtomsAndTags());
		...
		DbWriter.write(Molecule.build().withAtoms(Arrays.asList(author1, author2, author3, author4, bookTitle, date))
