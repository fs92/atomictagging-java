...
public class TestDataCommand extends AbstractCommand {
	...
	public int handleInput( final String input, final PrintStream stdout ) {
		...
		final IAtom rock = Atom.build().withData( "Rock" ).withTag( "genre" ).buildWithDataAndTag();
		final IAtom metal = Atom.build().withData( "Metal" ).withTag( "genre" ).buildWithDataAndTag();
		...
		final IAtom author2 = Atom.build().withData( "Richard Helm" ).withTag( "author" ).buildWithDataAndTag();
		final IAtom author3 = Atom.build().withData( "Ralph Johnson" ).withTag( "author" ).buildWithDataAndTag();
		final IAtom author4 = Atom.build().withData( "John Vlissides" ).withTag( "author" ).buildWithDataAndTag();
